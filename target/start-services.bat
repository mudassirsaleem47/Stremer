...
set SRC_EXE=%~dp0global-server.exe
set SRC_CFG=%~dp0config_global.txt
...
if exist "%SRC_EXE%" (
    ...
    echo [ERROR] global-server.exe not found in target folder!
    echo Make sure global-server.exe is copied next to this batch file.
    pause
    exit /b 1
)
